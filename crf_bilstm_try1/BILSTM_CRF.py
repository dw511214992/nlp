...
class BILSTM_CRF(object):
    ...
    def __init__(self, num_chars, num_classes, num_steps=200, num_epochs=100, embedding_matrix=None, is_training=True, is_crf=True, weight=False):
        ...
        if not is_crf:
            self.loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(self.targets, depth=14), logits=self.logits)
        else:
    ...
    def forward(self, observations, transitions, length, is_viterbi=True, return_best_seq=True):
        ...
        transitions = tf.reshape(tf.concat([transitions] * self.batch_size, 0), [self.batch_size, transitions.shape[0], transitions.shape[1]])
